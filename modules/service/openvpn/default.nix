{
    ...
    config = lib.mkIf config.service.openvpn.enable {
        assertions = [
            {
        ...
        sops = lib.mkIf config.sops.enable {
            ...
            templates = {
                "PIA-login" = lib.mkIf (config.service.openvpn.PIA || config.service.openvpn.PIAqBittorrentService) {
                    content = ''
        ...
        in {
            ...
            servers = {
                ...
                PIA-qBittorrent = lib.mkIf config.service.openvpn.PIAqBittorrentService {
                    config = ''
                        config ${toString ./servers/PIA_melbourne.ovpn}
