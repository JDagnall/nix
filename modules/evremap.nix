{
    ...
    options = {
        evremap.enable = lib.mkEnableOption {
            description = "Enable evremap, a key remapper that works in wayland. By Wez.";
        };
        evremap.profile = lib.mkOption {
            type = lib.types.enum ["keychron-c3" "framework" "macbook" "logi_k855"];
    ...
    in
        lib.mkIf config.evremap.enable {
            services.evremap = {
                enable = true;
                settings = profiles.${config.evremap.profile};
            };
        };
}
