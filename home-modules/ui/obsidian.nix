{
    ...
    config = lib.mkIf config.ui.obsidian.enable {
        programs.obsidian = {
            enable = true;
            cli.enable = true;
            # nix cannot manage vaults that are shared, as the symlinks
            # would obviously break on another machine. Currently I do not
            ...
            defaultSettings = {
                ...
                # files, that are needed by plugins, not using atm.
                # extraFiles = {
                # 	latex-preamble = {
                # 		target = "preamble.sty";
                # 		source = ./preamble.sty;
                ...
                # the fetchers for these are all busted, they should all be getting a zip
                # provided in the releases of the githubs, not the source code, but the idea
                # would be the same
                # communityPlugins = [
                # 	{
                # 		pkg =
